...
impl LzPointer {
    ...
    pub const fn new(len: usize, off: usize) -> Self {
    ...
    pub fn read<R: Read>(bitreader: &mut BitReader<R>) -> Result<Self, std::io::Error> {
        ...
        // Zero ptr, we can skip offset bytes
        if len == 0 {
    ...
    /// Find longest match that starts before `input_pos`
    pub fn find(input: &[u8], input_pos: usize) -> Result<Self, std::io::Error> {
        let search_start = input_pos.saturating_sub(Self::MAX_OFF);

        let mut best_len = 0;
        let mut best_off = 0;

        for off in search_start..input_pos {
        ...
        // Pointing back to 1 byte long matches actually wastes space.
        if best_len < 2 {
            return Ok(Self::literal());
        }
...
mod tests {
    ...
    fn test_pointer_find_match() {
        ...
        assert_eq!(result, LzPointer::new(3, 8));
    }
    ...
    fn test_pointer_find_match_exceed() {
        ...
        assert_eq!(result, LzPointer::new(7, 5));
    }

    // Tests match minimum limit.
    #[test]
    fn test_pointer_find_match_3b() {
        ...
        assert_eq!(result, LzPointer::new(3, 6));
    }
    ...
    fn test_pointer_find_match_2b() {
        ...
        assert_eq!(result, LzPointer::new(2, 6));
    }

    // Tests match minimum limit.
    #[test]
    fn test_pointer_find_match_1b() {
        let input = b"abcdeffxghifyjklmn";
    ...
    fn test_pointer_read_some() {
        ...
        assert_eq!(
            LzPointer::read(&mut bitreader).unwrap(),
            LzPointer::new(0b_0100, 0b_111100001111)
        );
    }
    ...
    fn test_pointer_read_write_cycle() {
        ...
        let ptr_a = LzPointer::new(0b_0101, 0b_0101_0001_1000);
        let ptr_b = LzPointer::new(0b_0001, 0b_0001_0000_0110);
        let ptr_c = LzPointer::new(0b_1101, 0b_0000_0000_0000);
        let ptr_d = LzPointer::new(0b_1110, 0b_1111_1111_1110);
        let ptr_e = LzPointer::new(0b_1111, 0b_1111_0110_0000);
        let ptr_f = LzPointer::new(0b_0110, 0b_0110_0011_0000);
        let ptr_g = LzPointer::new(0b_0111, 0b_0111_0000_0111);
